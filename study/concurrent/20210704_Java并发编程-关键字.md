...
代码中采用双重检查加锁（DCL）的方式来实现单例模式，第一个检查是为了防止每次getInstance的时候都要加锁，提高效率，第二个检查是为了防止多个线程先后进入锁内，如果不检查就会导致类被多次创建，则无法达到单例的效果。

此处单例对象使用volatile关键字修饰，主要为了防止指令的重排序，在对象构造过程主要分为以下三个步骤：

+ 分配内存空间；
+ 初始化对象；
+ 将内存对象的地址赋值给对应的引用；

操作系统可以对指令进行重排序，上面的过程就可能变成如下过程：
...
每个线程拥有自己的一个高速缓冲器：线程的工作内存，这样就可以导致每个线程的工作内存没有及时刷新，出现一个线程修改了共享变量的值，另一个线程不可见的问题。volatile关键字基于内存屏障实现了可见性。

#### 保证单次操作的原子性

volatile仅能保证单次操作的原子性，如`i++`操作就不能保证整体原子性，此时就必须要加锁才能实现功能。
...
当某个类整体定义为final时，就表明不能继承该类，且它的所有方法都隐式的为final的，因此无法覆盖他们。
...
Java允许在参数列表中以声明的方式将参数指明为final，这意味着无法在方法中更改参数引用所指向的对象，这个特性主要用于向匿名内部类传递数据。
